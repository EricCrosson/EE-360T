...
public class MinAndMaxWebTestGenerator {
    ...
    public static String generateTest(String test) {
        ...
        if (isx && isy && isz) {
            ...
            if (minormax.equals("min")) {
                mini = Math.min(Integer.parseInt(x), Integer.parseInt(y));
                mini = Math.min(mini, Integer.parseInt(z));
            } else {
                mini = Math.max(Integer.parseInt(x), Integer.parseInt(y));
                mini = Math.max(mini, Integer.parseInt(z));
            }
            min = minormax + "("+x+", "+y+", "+z+") = " + mini.toString();
        } else {
        ...
        "// execute the test <x = "+x+", y = "+y+", z = "+z+", submitButton = "+mouse+"lick>\n" +
            ...
            "we = driver.findElement(By.id(\""+minormax+"\"));\n" +
            "we.click();\n" +
    ...
    public static String[] getTestLists() {
        ...
        String[] quarterList = getAllLists(partitions, numFields);
        /* duplicate list for click and no click tests */
        String[] halfList = new String[2*quarterList.length];
        for(int i = 0; i < quarterList.length; ++i) {
            String test = quarterList[i];
            halfList[2 * i] = String.format("%sclick,", test);
            halfList[2 * i + 1] = String.format("%snotclick,", test);
        }
        String[] fullList = new String[2*halfList.length];
        for(int i = 0; i < halfList.length; ++i) {
            String test = halfList[i];
            fullList[2 * i] = String.format("%smin", test);
            fullList[2 * i + 1] = String.format("%smax", test);
        }
        return fullList;
    }
